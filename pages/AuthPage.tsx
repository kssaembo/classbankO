...
const AuthPage: React.FC = () => {
    ...
    const handleAdminLogin = async () => {
        ...
        let isValid = false;
        // 역할을 기준으로 소스 코드 내 하드코딩된 비밀번호와 비교
        if (loginTarget.role === Role.TEACHER) {
            isValid = password === 'admin123';
        } else if (loginTarget.role === Role.BANKER) {
            isValid = password === '1234';
        } else if (loginTarget.role === Role.MART) {
            isValid = password === '5678';
        }
