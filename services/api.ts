...
const transfer = async (senderUserId: string, recipientAccountNumber: string, amount: number, memo?: string, forcedRecipientPkId?: string): Promise<string> => {
    ...
    let recipientPkId: string | null = forcedRecipientPkId || null;

    // PK ID를 모르는 경우에만 조회를 시도합니다.
    if (!recipientPkId) {
        // [수정] 국고 계좌(K-국고-01) 조회 시 존재하지 않는 'id' 컬럼 대신 'accountId' 컬럼을 조회하도록 수정했습니다.
        if (recipientAccountNumber === 'K-국고-01' || recipientAccountNumber === 'K-국고-01'.trim()) {
            try {
                const { data: teacherAcc } = await supabase
                    .from('accounts')
                    .select('accountId') // 'id' -> 'accountId'로 수정
                    .eq('userId', 'teacher-01')
                    .single();
                if (teacherAcc) {
                    recipientPkId = teacherAcc.accountId; // .id -> .accountId로 수정
                }
            } catch (e) {
                console.warn("Teacher lookup failed during transfer");
            }
        }

        // 국고 계좌 조회가 안 되었거나 일반 학생 계좌인 경우 accountId로 조회합니다.
        if (!recipientPkId) {
            const { data: recipientAcc, error: lookupError } = await supabase
                .from('accounts')
                .select('accountId') // 'id' -> 'accountId'로 수정
                .eq('accountId', recipientAccountNumber.trim())
                .single();
            
            if (lookupError || !recipientAcc) {
                console.error("Recipient lookup failed:", lookupError);
                throw new Error("상대방 계좌를 찾을 수 없습니다.");
            }
            recipientPkId = recipientAcc.accountId;
        }
    }

    const { data, error } = await supabase.rpc('transfer_funds', {
        p_sender_user_id: senderUserId,
        p_receiver_account_pk_id: recipientPkId,
